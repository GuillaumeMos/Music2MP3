# gui.py
import os, csv, platform, tempfile, threading, queue, tkinter as tk
...
class Spotify2MP3GUI:
    def __init__(self, root: tk.Tk):
        ...
        self._total_tracks = 0  # fixé par 'conv_init'
    ...
    # ---------- Conversion (thread) + per-track UI ----------
    ...
    def start_conversion(self):
        ...
        self._clear_track_list()  # reset per-item UI
    ...
    def _run_conversion_worker(self):
        try:
            conv = Converter(
                ...
                item_cb=lambda k, d: self._conv_q.put(('item', (k, d))),  # NEW: per-track events
            )
        ...
        try:
            while True:
                ...
                if kind == 'status':
                    ...
                    # on ignore pour le global (on lisse via per-track), mais on peut garder le maximum
                    _cur, _maxi = payload
                elif kind == 'item':
                    ev, data = payload
    ...
    def _handle_item_event(self, ev: str, d: dict):
        if ev == 'conv_init':
            self._total_tracks = int(d.get('total', 0))
            self._perc.clear()
            # global progress in 0.. total*100
            self.progress.configure(mode='determinate', maximum=max(1, self._total_tracks * 100), value=0)
            return

        if ev == 'init':
            idx = int(d['idx']); title = d.get('title') or f"Track {idx}"
            self._ensure_row(idx, title)
            self._set_percent(idx, 0.0)
            return

        if ev == 'progress':
            idx = int(d['idx']); p = float(d.get('percent', 0.0))
            self._set_percent(idx, p)
            # update label text with small meta
        ...
        if ev == 'done':
            ...
            row = self._rows.get(idx); 
            if row: row['label'].config(text=f"{idx:03d}. {row['title']}  (100 %)")
            return
        ...
        if ev == 'error':
            ...
            self._set_percent(idx, 100.0)  # on compte comme terminé pour la globale
            return
    ...
    def _ensure_row(self, idx: int, title: str):
        ...
        lbl.pack(fill='x')
        bar = ttk.Progressbar(frame, orient='horizontal', length=680, mode='determinate', maximum=100, value=0)
    ...
    def _set_percent(self, idx: int, p: float):
        ...
        # recalcul de la globale
        if self._total_tracks:
            s = sum(self._perc.get(i, 0.0) for i in self._perc)
            self.progress.configure(value=s)  # max déjà = total*100
